...
<head>
    ...
    <style>
        ...
        body {
            ...
            /* Fuente monoospaciada para mejor alineación */
            font-size: 14px;
            width: 21cm;
            /* Ancho A4 Aprox */
            height: 29.7cm;
...
<body onload="window.print()">
    ...
    @php
        $d = \Carbon\Carbon::parse($workOrder->entry_date); // O fecha factura?
    @endphp

    <!-- FECHA -->
    <div class="field date-day">{{ $d->format('d') }}</div>
    <div class="field date-month">{{ $workOrder->month_text ?? $d->format('M') }}</div>
    ...
    <!-- NUMERO FACTURA (Control) -->
    <div class="field invoice-number-check">{{ $workOrder->invoice_number }}</div>

    <!-- CLIENTE -->
    <div class="field client-ruc">{{ $workOrder->vehicle?->customer?->document ?? '---' }}</div>
    <div class="field client-name">{{ $workOrder->vehicle?->customer?->name ?? 'Cliente Desconocido' }}</div>
    <div class="field client-address">{{ $workOrder->vehicle?->customer?->address ?? 'Asunción' }}</div>
    <div class="field client-phone">{{ $workOrder->vehicle?->customer?->phone ?? '---' }}</div>

    <!-- ITEMS -->
    <div class="items-container">
        <!-- Servicios -->
        @foreach ($workOrder->services as $svc)
            <div class="item-row">
    ...
    <div class="field total-number">{{ number_format($workOrder->total_price, 0, ',', '.') }}</div>

    <!-- LIQUIDACION IVA (Asumiendo todo 10%) -->
    @php
        $iva10 = round($workOrder->total_price / 11);
    @endphp
